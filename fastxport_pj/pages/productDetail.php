...
$sql = "SELECT p.id_product, p.product_name, p.description, p.media, p.price, p.stock, p.minBuy, s.shop_name, l.user_add AS location
        FROM product p
...
<div class="Product-Info">
    ...
    <div class="Product-Info-right">
        ...
        <?php if ($address): ?>
            <div class="address">
                <span>Location: <?php echo htmlspecialchars($address); ?></span>
            </div>
        <?php endif; ?>
        <form action="invoice.php" method="GET">
            <input type="hidden" name="id" value="<?php echo htmlspecialchars($product['id_product']); ?>">
            <div class="quantity-selector">
                <button type="button" class="decrease" onclick="updateQuantity(-1)">-</button>
                <input type="number" id="quantity" name="qty" value="1" min="<?php echo htmlspecialchars($product['minBuy']); ?>" max="<?php echo htmlspecialchars($product['stock']); ?>">
